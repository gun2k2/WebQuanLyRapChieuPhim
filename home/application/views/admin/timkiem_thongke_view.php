		...
		<table>
			...
			<thead >
				<tr class="text-center">
					...
					<!-- <td style="width: 20%;">Phòng chiếu</td> -->
					
					<td style="width: 20%;">Ngày mua</td>
					
					
					<td style="width: 10%;">Mã vé</td>
					<td style="width: 10%;">Thông tin người mua</td>
					<td style="width: 10%;">Giá vé</td>
...
<!-- Dòng cần thiết để chuyển số thành chữ -->
					<?php $CI =& get_instance(); 
						if ($this->ketqua != null) $this->load->view('admin/message_view', $this->ketqua);
...
<!--END Dòng cần thiết để chuyển số thành chữ -->					
				...
				<?php foreach ($mangketqua as  $value => $key ) : ?>
				<!-- table row -->
				<tr class="text-center">
					
					<td style="width: 20%;"><?= $CI->showphim_model->ktdanhmuc($key['tenphim'])  ?></td>
					
					
					
					
					<td style="width: 10%;"><?=$key['ngaymua'] ?></td>
						
					<td style="width: 10%;"><?=$key['ma_ve'] ?></td>
					<?php if($key['email'] == null): ?>
						<td style="width: 10%;">Admin</td>
					<?php else: ?> <td style="width: 10%;"><?=$key['email'] ?></td>
					<?php endif ?>
					<td style="width: 10%;"><?=number_format($key['giave']) ?></td>
				
				</tr>
				<?php $tongcong = $tongcong + $key['giave']?>
				<!-- end table row -->
				<?php endforeach ?>
